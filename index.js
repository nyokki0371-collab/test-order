...
function addMyTicket(orderId) {
  const tickets = getMyTickets();
  if (!tickets.includes(orderId)) {
    tickets.push(orderId);
...
// 公序良俗に反するNGワードリスト（ひらがな基準でチェック）
const BANNED_WORDS = [
  "うんこ", "うんち", "おしっこ", "ちんこ", "まんこ", "ちんちん", "ちんぽ", "ちんぶ", "おめこ", 
  "ぺにす", "ゔぁぎな", "せっくす", "まざふぁか", "しっこ", "うんご", "くりとりす", "ふぇら", 
  "くんに", "ばいぶ", "ろーたー", "ちつ", "にゅうりん", "ごむ", "こんどーむ",
  "unko", "unchi", "chinko", "manko", "chinchin", "penis", "vagina", "sex", "porn", "pussy", "chinpo", "condom", "fela"
];
...
function checkBannedWords(text) {
  // 小文字化、全角スペースを含む空白除去、カタカナをひらがなへ正規化
...
orderForm.addEventListener("submit", async (e) => {
  ...
  // 名前バリデーション（空チェック ＆ NGワードチェック）
  if (!name) {
    nameError.textContent = "名前(ニックネーム)を入力してください";
    nameError.style.display = "block";
    hasError = true;
  } else if (checkBannedWords(name)) {
    nameError.textContent = "不適切な単語が含まれています。別の名前を入力してください。";
    nameError.style.display = "block";
    hasError = true;
  } else {
  ...
  try {
    ...
    // 自分の注文IDとしてローカルストレージに保存
    addMyTicket(newOrderId);
