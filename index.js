  ...
  try {
    ...
    await Promise.race([
      runTransaction(db, async (transaction) => {
        ...
        nextTicket = currentTicket + 1;
        if (nextTicket > 9999 || nextTicket < 1 || isNaN(nextTicket)) {
          nextTicket = 1;
        }
    ...
    if (pickupInstruction) {
      if (location === "店舗受け取り") {
        pickupInstruction.textContent = "【販売所にて整理券番号とニックネームを教えてください】";
      } else {
        pickupInstruction.textContent = "【販売員がお届けに参り、整理券番号とニックネームをお呼びするので速やかにお受け取り下さい】";
      }
    }
